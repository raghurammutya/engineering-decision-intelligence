# V2 Portfolio Summary

Generated: `2026-05-23T05:32:41+00:00`

Portfolio: `engineering-decision-intelligence-v2`
Repositories: `2`
Total scanned artifacts: `573`
Repositories with blocked autonomy: `2`

| repo_id | role | owner | scan_observed | artifact_count | dirty_file_count |
| --- | --- | --- | --- | --- | --- |
| edi-product | product | product-platform | True | 6 | 0 |
| ml-pilot | pilot-customer-system | ml-platform | True | 567 | 2 |
